...
const ExerciseDetail: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'instruction' | 'about'>('instruction');
          ...
          <div className="bg-gym-surface rounded-3xl p-8 border border-white/5">
            ...
            <div className="flex space-x-6 mb-8 border-b border-white/10">
              <button
                ...
                className={`pb-4 text-sm font-bold uppercase tracking-widest transition-all ${activeTab === 'instruction' ? 'text-gym-accent border-b-2 border-gym-accent' : 'text-gray-500 hover:text-white'}`}
              >
                Instruksi
              </button>
              <button
                onClick={() => setActiveTab('about')}
                className={`pb-4 text-sm font-bold uppercase tracking-widest transition-all ${activeTab === 'about' ? 'text-gym-accent border-b-2 border-gym-accent' : 'text-gray-500 hover:text-white'}`}
              >
                Tentang & Manfaat
              </button>
            </div>
              ...
              <div className="space-y-6">
                <div className="prose prose-invert max-w-none">
                  <p className="text-gray-300 leading-relaxed text-lg mb-6">
                    Penjelasan detail mengenai gerakan <strong>{exercise.name}</strong>. Pelajari fungsi, manfaat, dan cara kerja otot saat melakukan latihan ini.
                  </p>
                  <div className="p-6 bg-white/5 rounded-2xl border border-white/10 mb-8">
                    <h4 className="text-white font-bold mb-3">Deskripsi Database</h4>
                    <div className="text-gray-400" dangerouslySetInnerHTML={{ __html: exercise.description }}></div>
                  </div>
                </div>

                <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 p-8 rounded-3xl border border-blue-500/20 text-center">
                  <h4 className="text-2xl font-black text-white mb-2">Ingin tahu lebih dalam?</h4>
                  <p className="text-gray-400 mb-6">Cari tahu manfaat spesifik, variasi, dan tips pro langsung dari sumber terpercaya.</p>

                  <a
                    href={`https://www.google.com/search?q=benefits+dan+fungsi+dari+${exercise.name}+exercise+anatomy`}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center space-x-3 bg-white text-black px-8 py-4 rounded-full font-black hover:bg-gym-accent transition-all transform hover:scale-105 shadow-xl shadow-white/10"
                  >
                    ...
                    <span>Tanya Google / AI</span>
                  </a>
                  <p className="text-[10px] text-gray-500 mt-4 tracking-widest uppercase">Powered by Google Search Engine</p>
                </div>
              </div>
            )}
          </div>
